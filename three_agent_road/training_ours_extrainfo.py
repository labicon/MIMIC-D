...
mean_orig3 = np.mean(orig3, axis=(0,1))
std_orig3 = np.std(orig3, axis=(0,1))
orig1 = (orig1 - mean_orig1) / std_orig1
orig2 = (orig2 - mean_orig2) / std_orig2
orig3 = (orig3 - mean_orig3) / std_orig3
...
obs2 = np.hstack([obs_init2, obs_final2, obs_init1, obs_init3])
obs3 = np.hstack([obs_init3, obs_final3, obs_init1, obs_init2])
obs_temp1 = obs1
obs_temp2 = obs2
obs_temp3 = obs3
...
for s in range(10):
    ...
    np.random.seed(seed)
    torch.manual_seed(seed)
    path = f"sampled_trajs/ours_extrainfo_seed{seed}"
    ...
    for i in range(100):
        ...
        final3 = (final3 - mean_orig3) / std_orig3

        planned_trajs = reactive_mpc_plan(action_cond_ode, [initial1, initial2, initial3], [final1, final2, final3], segment_length=H, total_steps=T, n_implement=1)
