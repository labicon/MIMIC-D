...
# 2) Hyperparameters
state_dim   = 8       # e.g. [x, y, other_agent1_x, other_agent1_y, other_agent2_x, other_agent2_y]
...
def build_sa(expert, other1, other2):
    ...
    for i in range(len(expert)):
        ...
        for t in range(len(traj) - 1):
            s = np.hstack([traj[t], traj[-1], other_traj1[t], other_traj2[t]])  # (state_dim,)
...
# 8) Save models
save_path_G1 = "trained_models/magail_ctde/G1_extrainfo.pth"
save_path_G2 = "trained_models/magail_ctde/G2_extrainfo.pth"
save_path_G3 = "trained_models/magail_ctde/G3_extrainfo.pth"
save_path_D  = "trained_models/magail_ctde/D_extrainfo.pth"
...
@torch.no_grad()
def rollout_two_agents(G1, G2, G3, init1, init2, init3, final1, final2, final3, T, state_mean, state_std, action_mean, action_std, device):
    cur1 = init1.copy(); cur2 = init2.copy(); cur3 = init3.copy()
    traj1 = [cur1.copy()]; traj2 = [cur2.copy()]; traj3 = [cur3.copy()]
    for _ in range(T-1):
        s1 = np.hstack([cur1, final1, cur2, cur3])  # other = current, not initial
        s2 = np.hstack([cur2, final2, cur1, cur3])
        s3 = np.hstack([cur3, final3, cur1, cur2])
...
for s in range(10):
    ...
    for i in range(num_samples):
        ...
        while True:
            ...
            if (d_init12 > threshold and d_init13 > threshold and d_init23 > threshold and d_final12 > threshold and d_final13 > threshold and d_final23 > threshold):
                break

        traj1, traj2, traj3 = rollout_two_agents(G1, G2, G3, initial1, initial2, initial3, final1, final2, final3, T, state_mean, state_std, action_mean, action_std, device)
    ...
    for _ in range(N):
        traj1, traj2, traj3 = rollout_two_agents(G1, G2, G3, init1, init2, init3, goal1, goal2, goal3, T, state_mean, state_std, action_mean, action_std, device)
