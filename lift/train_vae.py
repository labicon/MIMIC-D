            ...
            try:
                with h5py.File(h5_path, "r") as f:
                    if self.data_key is not None and self.data_key in f:
                        keys_to_use = [self.data_key]
                    else:
                        keys_to_use = [name for name in self.type_names if name in f]

                    if not keys_to_use:
                        continue

                    # add all matching keys from this file
...
def train_vae(vae_model, data_loaders, model_save_path, log_images_to_path, epochs, learning_rate, image_keys, device):
    ...
    for epoch in tqdm_epoch:
        ...
        with torch.no_grad():
            output = vae_model(test_images.to(device)).sample.detach().cpu()
    ...
    best_model_state = torch.load(f'{model_save_path}/best_vae.pth', map_location=device)
...
def main():
    ...
    parser.add_argument('--save_path', type=str, default=os.path.join(os.path.dirname(__file__), 'data/models/VAE_models_ICON'))
    parser.add_argument('--validation_image_out_dir', type=str, default=os.path.join(os.path.dirname(__file__), 'data/vae_val_images'))
    parser.add_argument('--log_dir', type=str, default=os.path.join(os.path.dirname("__file__"), 'data/logs/'))
    parser.add_argument('--h5_dir', type=str, default=os.path.join(os.path.dirname("__file__"), 'data/h5data/'))
    parser.add_argument('--image_keys', type=str, nargs='+', default=['robot0_eye_in_hand_image', 'robot1_eye_in_hand_image'])
    ...
    vae_model = AutoencoderKL.from_pretrained("stabilityai/sd-vae-ft-ema").to(DEVICE)
...
#!python vae_testerencoder.py   --h5_dir data/h5data/   --image_keys robot0_eye_in_hand_image robot1_eye_in_hand_image   --batch_size 64   --epochs 50