...
class ContinuousCondEmbedder(nn.Module):
    """Embed per-timestep continuous attributes (state + image latent)."""
    
    def __init__(self, attr_dim: int, hidden_size: int, lin_scale: int):
        super().__init__()
        self.attr_dim = attr_dim
        self.lin_scale = lin_scale
        self.embedding = nn.Linear(attr_dim, int(attr_dim * lin_scale))  # affine per timestep
        self.attn = nn.MultiheadAttention(int(attr_dim * lin_scale), num_heads=2, batch_first=True)
        self.linear = nn.Linear(int(attr_dim * lin_scale), hidden_size)
    
    def forward(self, attr: torch.Tensor, mask: Optional[torch.Tensor] = None):
        """
        attr: (batch, H, attr_dim)
        mask: (batch, H, attr_dim), optional
        """
        b, H, D = attr.shape
        emb = self.embedding(attr)  # (b, H, attr_dim*lin_scale)
        if mask is not None:
            emb *= mask  # broadcast mask (b, H, D)
        emb, _ = self.attn(emb, emb, emb)  # (b, H, attr_dim*lin_scale)
        return self.linear(emb)  # (b, H, hidden_size)
...
class DiT1d(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor,
        ...
        # Expand timestep embedding to horizon
        t_emb = self.t_emb(t).unsqueeze(1)  # (batch, 1, hidden_size)

        if attr is not None:
            attr_emb = self.attr_proj(attr, mask)  # (batch, H, hidden_size)
            t_emb = t_emb + attr_emb  # broadcast addition along horizon
...
class Conditional_ODE():
    ...
    def save(self, extra: str = ""):
        ...
        torch.save(state, "data/models/DiffusionModels/" + self.filename + extra + ".pt")
        
    def load(self, extra: str = ""):
        """Loads state dictionaries for all transformers and their EMA copies."""
        name = "data/models/DiffusionModels/" + self.filename + extra + ".pt"
