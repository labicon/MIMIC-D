...
class PolicyPlayer:
    ...
    def load_model(self, expert_data1, expert_data2, obs_init1, obs_init2, obs, image_latents_initial_arm1, image_latents_initial_arm2, state_dim = 7, action_dim = 7):
        ...
        obs1 = np.hstack([image_latents_initial_arm1])
        obs2 = np.hstack([image_latents_initial_arm2])
        ...
        action_cond_ode.load(extra="_lift_mpc_P15E1_encodedImageAndState_rotvec_separatenorm_dual_camera")
    ...
    def reactive_mpc_plan(self, ode_model, initial_states, pot, image_latents_initial_arm1, image_latents_initial_arm2, segment_length=25, total_steps=325, n_implement=2):
        ...
        - initial_states: list of initial normalized states for each arm
        - pot: pot position (not used in image-only model)
        - image_latents_initial_arm1: full sequence of image latents for arm1 (T, latent_dim)
        - image_latents_initial_arm2: full sequence of image latents for arm2 (T, latent_dim)
        - segment_length: number of timesteps to plan in each segment.
        - total_steps: total length of the planned trajectory.
        - n_implement: number of steps to execute before replanning

        Returns:
        - full_traj: a numpy array of shape (2, total_steps, state_size)
        """
        full_traj = []
        current_states = initial_states.copy()
        current_timestep = 0  # Track current timestep for image latent indexing

        for seg in range(total_steps // n_implement):
            segments = []
            base_states = current_states.copy()

            # Get the image latents for the current timestep (matching training behavior)
            # During training, each sub-trajectory uses the image from its start timestep
            current_image_latent_arm1 = image_latents_initial_arm1[min(current_timestep, len(image_latents_initial_arm1)-1)]
            current_image_latent_arm2 = image_latents_initial_arm2[min(current_timestep, len(image_latents_initial_arm2)-1)]
            ...
            for i in range(len(base_states)):
                ...
                if i == 0:
                    cond = np.hstack([current_image_latent_arm1])
                else:
                    cond = np.hstack([current_image_latent_arm2])
            ...
            # Increment timestep counter for next iteration
            current_timestep += n_implement
    ...
    def get_demo(self, seed, cond_idx, H=25, T=700):
        ...
        # Loading
        expert_data = np.load("TrainingDataDiffusion_image+state/expert_actions_newslower_20.npy")
        expert_data1 = expert_data[:, :, :7]
        expert_data2 = expert_data[:, :, 7:14]

        # Load original image sequences BEFORE creating MPC dataset
        expert_images_latents_arm1_orig = np.load("TrainingDataDiffusion_image+state/arm1_images_latents.npy")
        expert_images_latents_arm2_orig = np.load("TrainingDataDiffusion_image+state/arm2_images_latents.npy")
        ...
        expert_images_latents_arm1 = create_mpc_dataset(expert_images_latents_arm1_orig, planning_horizon=H)
        expert_images_latents_arm2 = create_mpc_dataset(expert_images_latents_arm2_orig, planning_horizon=H)
        ...
        with open("TrainingDataDiffusion_image+state/pot_start_newslower_20.npy", "rb") as f:
            obs = np.load(f)
        ...
        traj_idx = cond_idx // T  # which trajectory
        start_timestep = cond_idx % T  # starting timestep within that trajectory

        # Extract the full sequence of image latents from the selected trajectory
        # Use the original sequences, not the MPC sub-trajectories
