...
def build_bc_data(expert_data):
    ...
    for traj in expert_data:
        ...
        for t in range(len(traj)-1):
            current = traj[t]
            X.append(np.hstack([current, final]))
            Y.append(traj[t+1])
...
# Save trained models
save_path1 = "trained_models/bc/model1.pth"
save_path2 = "trained_models/bc/model2.pth"
save_path3 = "trained_models/bc/model3.pth"
...
model3 = ImitationNet(input_size=4, hidden_size=64, output_size=2).to(device)
...
for i in range(n_samples):
    ...
    traj2 = [init2]
    traj3 = [init3]
    s1 = torch.from_numpy(np.hstack([init1, fin1])).float().to(device).unsqueeze(0)
    s2 = torch.from_numpy(np.hstack([init2, fin2])).float().to(device).unsqueeze(0)
    s3 = torch.from_numpy(np.hstack([init3, fin3])).float().to(device).unsqueeze(0)
    ...
    for _ in range(traj_length-1):
        ...
        traj2.append(next2)
        traj3.append(next3)
        s1 = torch.from_numpy(np.hstack([next1, fin1])).float().to(device).unsqueeze(0)
        s2 = torch.from_numpy(np.hstack([next2, fin2])).float().to(device).unsqueeze(0)
        s3 = torch.from_numpy(np.hstack([next3, fin3])).float().to(device).unsqueeze(0)

    generated[1].append(np.array(traj1))
    np.save(f"sampled_trajs/bc/traj1_{i}.npy", np.array(traj1))
    generated[2].append(np.array(traj2))
    np.save(f"sampled_trajs/bc/traj2_{i}.npy", np.array(traj2))
    generated[3].append(np.array(traj3))
    np.save(f"sampled_trajs/bc/traj3_{i}.npy", np.array(traj3))
